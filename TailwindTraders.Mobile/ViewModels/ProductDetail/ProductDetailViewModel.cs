...
namespace TailwindTraders.Mobile.Features.Product.Detail
{
    public class ProductDetailViewModel : BaseStateAwareViewModel<ProductDetailViewModel.State>
    {
        ...
        private IEnumerable<Models.Product> alsoBoughtProducts;
        ...
        public IEnumerable<Models.Product> AlsoBoughtProducts
        {
        ...
        private async Task RequestProductDetailAsync()
        {
            ...
            if (product != null)
            {
                ...
                var productsPerCategoryTask = productService.GetProductsPerCategory(product.Type.Code);
                var alsoBoughtProductsTask = productService.GetProductsAsync();

                await Task.WhenAll(
                    //qtyTask
                    productsPerCategoryTask, alsoBoughtProductsTask
                );
        ...
        private void UpdateProduct(Models.Product product, long qty)
        {
            var brandName = product.Brand.Name;
            var productName = product.Name;
            Title = $"{brandName}. {productName}";
            Pictures = new List<string> { product.RevisedImageUrl };
            Brand = brandName;
            Name = productName;
            Price = $"${product.Price}";
